...
static char remote_device_name[20];
static uint8_t remote_device_bda[6];

static bool connect    = false;
static bool get_server = false;
static bool get_data   = false;
uint8_t *notify_vlaue = NULL;
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
        ...
        if (memcmp(p_data->search_res.srvc_id.uuid.uuid.uuid128, remote_filter_service_uuid.uuid.uuid128, ESP_UUID_LEN_128) == 0) {
    ...
    ESP_LOGE(GATTC_TAG, "in ESP_GATTC_NOTIFY_EVT");
        ...
        notify_vlaue = malloc(notify_len * sizeof(uint8_t));
        for(int i = 0 ; i < p_data->notify.value_len ; i++) {
            notify_vlaue[i] = p_data->notify.value[i];
        }
        get_data = true;
        break;
...
void init_BLE(char *name, uint8_t *bda,  uuid_t myUUIDs)
{
    ...
    memcpy(remote_device_bda, bda, 6);
    strcpy(remote_device_name, name);

    if(myUUIDs.service_uuid_type == 2)
    {
        remote_filter_service_uuid.len = ESP_UUID_LEN_128;
        memcpy(remote_filter_service_uuid.uuid.uuid128, myUUIDs.service_uuid, ESP_UUID_LEN_128);
    }
    if(myUUIDs.char_uuid_type == 2)
    {

        remote_filter_char_uuid.len = ESP_UUID_LEN_128;
        memcpy(remote_filter_char_uuid.uuid.uuid128, myUUIDs.char_uuid, ESP_UUID_LEN_128);
    }
